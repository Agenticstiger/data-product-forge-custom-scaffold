...
def test_pattern_entry_docstring_marks_unenforced_fields():
    doc = (PatternEntry.__doc__ or "").lower()
    assert "advisory" in doc, "supported_* must be documented as advisory metadata"
    assert "reserved" in doc, "variables_schema must be documented as reserved"
